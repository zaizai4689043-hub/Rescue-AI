...
function buildMapOption() {
  ...
  const scatterData = data.map(d => {
    const isSel = d.id === selectedId.value
    return {
      name: d.title,
      value: [d.longitude, d.latitude, d.severity],
      itemStyle: {
        color: TYPE_COLORS[d.disaster_type] || '#F56C6C',
        borderColor: isSel ? '#FFFFFF' : 'transparent',
        borderWidth: isSel ? 2 : 0,
      },
      symbolSize: isSel ? Math.max(d.severity * 5, 10) + 8 : undefined,
      _raw: d,
    }
  })
...
function initMap() {
  ...
  mapChart.on('click', 'series.effectScatter', (params) => {
    ...
    selectedId.value = params.data._raw?.id ?? null
    drawerVisible.value = true
  })
}

/** 按 id 选中灾情（供路由 query 联动） */
function selectById(id) {
  const found = allData.value.find(d => d.id === Number(id))
  if (!found) return
  selectedDisaster.value = found
  selectedId.value = found.id
  if (mapChart) mapChart.setOption(buildMapOption())
  drawerVisible.value = true
}
...
onMounted(async () => {
  ...
  initPie()
  // 路由联动：?id=<n> 自动选中并开抽屉
  if (route.query.id) selectById(route.query.id)
