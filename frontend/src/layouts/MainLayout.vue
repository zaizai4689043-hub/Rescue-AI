    ...
    <div class="deck-body">
      <header class="deck-header">
        ...
        <div class="header-right">
          ...
          <!-- 数字员工值守徽标 -->
          <div class="agent-chip" :class="{ flash: sentinelFlash }">
            <span class="agent-emoji">🛰</span>
            <span class="agent-label">哨兵</span>
            <span class="agent-dot green"></span>
          </div>
          <div class="agent-chip" :class="{ alert: pendingCount > 0 }">
            <span class="agent-emoji">🧠</span>
            <span class="agent-label">参谋</span>
            <span class="agent-dot yellow"></span>
          </div>
          <div class="agent-chip">
            <span class="agent-emoji">📡</span>
            <span class="agent-label">通讯员</span>
            <span class="agent-dot blue"></span>
          </div>
          <button class="bigscreen-btn" @click="$router.push('/command-screen')">⛶ 大屏模式</button>
...
// 数字员工徽标：待批准数 + 哨兵闪烁（近 2s 内有情报注入）
const pendingCount = computed(() => rescueStore.pendingCount)
const sentinelFlash = computed(() => nowTick.value - rescueStore.lastIngestAt < 2000)
...
.agent-emoji { font-size: 14px; line-height: 1; }
.agent-label { font-size: 12px; color: var(--deck-text-2); }
.agent-dot { width: 6px; height: 6px; border-radius: 50%; }
.agent-dot.green { background: var(--deck-teal); box-shadow: 0 0 5px var(--deck-teal); }
.agent-dot.yellow { background: var(--deck-amber); box-shadow: 0 0 5px var(--deck-amber); }
.agent-dot.blue { background: var(--deck-cyan); box-shadow: 0 0 5px var(--deck-cyan); }
@keyframes agentFlash { 0%, 100% { box-shadow: none; } 50% { box-shadow: 0 0 14px 3px rgba(56, 225, 255, 0.6); border-color: var(--deck-cyan); } }
.agent-chip.flash { animation: agentFlash 0.5s ease; }
.agent-chip.alert { border-color: rgba(255, 176, 32, 0.5); background: rgba(255, 176, 32, 0.1); }
...
.bigscreen-btn:hover { background: rgba(56, 225, 255, 0.18); box-shadow: 0 0 12px rgba(56, 225, 255, 0.3); }
