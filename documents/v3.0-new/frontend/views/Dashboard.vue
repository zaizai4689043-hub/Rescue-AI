...
import { DISASTER_TYPES, DISASTER_STATUS, DISASTER_TYPE_MAP, SEVERITY_LABELS, SEVERITY_COLORS } from '../utils/constants'

use([CanvasRenderer, LineChart, PieChart, TitleComponent, TooltipComponent, LegendComponent, GridComponent])

const stats = reactive({ total: 0, processing: 0, resolved: 0, today: 0, rescue_requests: 0 })
...
onMounted(async () => {
  try {
    ...
    Object.keys(stats).forEach(k => animateNumber(k, stats[k]))
  ...
  try {
    const listRes = await getDisasters({ page: 1, page_size: 5 })
